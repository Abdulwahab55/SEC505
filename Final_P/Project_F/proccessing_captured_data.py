...
def processing(File_name):
    ...
    for i in range(len(csv_files)):
        ...
        with open("flow_data/"+csv_files[i], "r") as file:
            while True:
                try:
                    line=file.readline()
        ...
        # if flag:
        #     df.to_csv('flow_data/Temp_traffic.csv' ,index = False)
        #     flag=False
        # else:
        #     df.to_csv('flow_data/Temp_traffic.csv' ,index = False,header=False,mode="a")
        # os.remove(str(i)+".csv")
        # print("The pre-processing phase of the ",csv_files[i]," file is completed.\n")
        
    
    df["Destination IP"] = x_d
    df["Source IP"] = x_s
