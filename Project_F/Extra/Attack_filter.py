...
for i in dict_attack: # in this section, a file is opened for each attack type and is recorded at a random benign flow.
    a,b=0,0
    ths = open(".\\attacks_datasets\\"+i + ".csv", "w")
...
flag=True
for i in webs:
    df=pd.read_csv(".\\attacks_datasets\\"+str(i)+".csv")
    if flag:
        df.to_csv('.\\attacks_datasets\\Web Attack.csv' ,index = False)
        flag=False
    else:
        df.to_csv('.\\attacks_datasets\\Web Attack.csv' ,index = False,header=False,mode="a")
    os.remove(".\\attacks_datasets\\"+str(i)+".csv")
